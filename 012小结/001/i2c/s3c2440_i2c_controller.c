...
void i2c_interrupt_func(int irq)
{
	...
	if(p_cur_msg->flags == 0)/* write */
	{
		...
		if(p_cur_msg->cnt_transferred == 0)/* 第一次中断 */
		{
		...
		if(p_cur_msg->cnt_transferred < p_cur_msg->len)/* 其他中断，继续发送数据 */
		{
...
int do_master_rx(p_i2c_msg msg)
{
	...
	IICCON |= (1<<7);/* rx mode,在ack周期释放sda */
	IICSTAT = (1<<4);
